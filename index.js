...
function getArtistByIndex(array, indexNum) {
  return `the artist at index ${array[indexNum].id} is ${array[indexNum].name}`
}
// uncomment line below to print
...
function get20s(array) {
  ...
  for(let i = 0; i < array.length; i++){
    let birthYear = Number(array[i].years.split('-')[0])
    let deathYear = Number(array[i].years.split('-')[1])
...
function addArtist(array) {
  array.push({
    ...
    bio: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Esse vitae aut quis cupiditate atque. Soluta deleniti voluptatum vel, ipsa adipisci quod suscipit ea expedita voluptates in consectetur totam odit quis.'
  })
  return array
}
